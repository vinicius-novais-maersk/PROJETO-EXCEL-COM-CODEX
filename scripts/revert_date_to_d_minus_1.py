...
def log(message: str) -> None:
    print(f"[revert-d-1] {message}", flush=True)
...
def main() -> int:
    ...
    try:
        ...
        for sheet_name in ["Volume_DS", "Volume_MAO", "Volume_Graph"]:
            ...
            set_value(ws, "L1", "D-1")
            set_formula(ws, "M1", "=WORKDAY(TODAY(),-1)")
            log(f"Restored D-1 header in {sheet_name}")

        set_formula(workbook.Worksheets("Week_Overview"), "AH1", "=WORKDAY(TODAY(),-1)")
        set_formula(workbook.Worksheets("Month2date"), "R1", "=WORKDAY(TODAY(),-1)")
        set_formula(workbook.Worksheets("LastMonth_Overview"), "R1", "=WORKDAY(TODAY(),-1)")
        log("Restored workbook date anchors to D-1")
