Sub ROE_wk_cancel()
    ...
    caminho = "C:\Users\RMA704\OneDrive - Maersk Group\2025\Py\DSU 2025\ROE_wk_cancel\ROE_wk_cancel.xlsx"
    Set wbFonte = Workbooks.Open(caminho, ReadOnly:=True, UpdateLinks:=0)
    Set wsFonte = wbFonte.Sheets("Planilha1")
    Set wsDestino = ThisWorkbook.Sheets("ROE_wk_cancel")
    ...
    ' 1) LIMPA TUDO ABAIXO DA LINHA 2
    '    Dados A:AK
    wsDestino.Range(wsDestino.Cells(3, "A"), wsDestino.Cells(wsDestino.Rows.Count, "AK")).ClearContents
    '    Fórmulas AL:BG
    wsDestino.Range(wsDestino.Cells(3, "AL"), wsDestino.Cells(wsDestino.Rows.Count, "BG")).ClearContents
    ...
    ' 4) PREENCHE FÓRMULAS (AL:BG) APENAS ATÉ AS NOVAS LINHAS
    formulaColStart = wsDestino.Range("AL2").Column
    formulaColEnd = wsDestino.Range("BG2").Column
    If qtdeLinhas > 0 Then
